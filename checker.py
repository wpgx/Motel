...
FULL_CA_US = Path("full_ca_us.m3u")  # NEW - full browsable pool
    ...
    try:
        ...
        for i,l in enumerate(lines):
            ...
            if l.startswith('#EXTINF'):
                low=l.lower()
                if not ('usa' in low or 'canada' in low or 'united states' in low or 'group-title="us"' in low or 'group-title="ca"' in low or 'group-title="usa"' in low or ' us ' in low): 
                    # broader but still CA/US
                    if 'group-title="ca:' not in low and 'group-title="us:' not in low and 'usa' not in low and 'canada' not in low:
                        continue
        ...
        # Write both temp filtered and final full file
        filtered_text = '\n'.join(kept)+'\n'
        BACKUP_FILTERED.write_text(filtered_text, encoding='utf-8')
        FULL_CA_US.write_text(filtered_text, encoding='utf-8')  # THIS IS THE FILE YOU POINT TO
        print(f" FILTERED to {cnt} CA/US -> {BACKUP_FILTERED.name} and {FULL_CA_US.name}")
...
def is_alive(url):
    ...
    try:
        ...
        if r.status_code in (403,401,404,500,502,503):
            print(f"  -> {r.status_code} dead master")
            return False
...
def main():
    print("=== checker - XUMO + FreeTV CA/US pool ===")
    ...
    FINAL.write_text('\n'.join(new_list)+'\n')
    print(f"DONE - {len(new_list)//2} chans in final_60.m3u")
    if FULL_CA_US.exists():
        print(f"FULL POOL - {FULL_CA_US.read_text().count('#EXTINF')} chans in full_ca_us.m3u ready to point to")
