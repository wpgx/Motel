...
def main():
    ...
    # pool of spares not in final and not welcome
    ...
    # --- ADD WELCOME AS CHANNEL 1 FIRST ---
    ...
    for ext,url in final:
        # skip if it's the old welcome entry to avoid duplicates
        if url == WELCOME_URL:
            continue
        if is_alive(url):
            ...
            if spare_idx < len(spares):
                ...
                new_list.append(ext); new_list.append(url)
    FINAL.write_text('\n'.join(new_list))
    print(f"Done. Replaced {replaced} channels. Welcome channel is now Ch 1.")
