...
export default {
  async fetch(request, env, ctx) {
    ...
    if (action === 'handshake' || action === 'do_handshake') {
      return new Response(JSON.stringify({ js: { token: "123", id: "1" } }), { headers });
    }
    if (action === 'get_profile') {
      return new Response(JSON.stringify({ js: { id: "1", name: "Motel" } }), { headers });
    }
    ...
    const js = {
      ...
      max_page_items: 1000,
      data: channels.map(c=>({ id: c.id.toString(), name: c.name, number: c.number.toString(), cmd: `ffmpeg ${c.cmd}`, logo: c.logo, tv_genre_id: "1" }))
    };
